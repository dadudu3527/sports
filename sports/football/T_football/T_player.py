...
class PlayerD:
    ...
    #     teamName2 = '^{}'.format(teamName)
    #     teamName3 = teamName[:2]
    #     note = self.col_read_team.find_one({'name_zh':teamName},{'teamId':1})
    #     if note:
    #         teamId = note.get('teamId')
    #     else:
    #         note = self.col_read_team.find_one({'$or':[{'name_zh':{'$regex':teamName2}},{'name_zh':{'$regex':teamName3}}]}, {'teamId': 1})
    #         if note:
    #             teamId = note.get('teamId')
    #         else:
    #             teamId = ''
    #     return teamId
    #
    # def get_player_id(self,resp): #resp是一个元组
    #     playerId = ''
    #     res,playerName,playerName_en = resp
    #     teamName = self.is_empty(self.my_xpath(res,'//div[@class="player-info"]/div/div[1]/table//tr[2]/td/text()'))
    #     note = self.col_read_team.find_one({'name_zh':teamName},{'teamId':1})
    #     if note:
    #         teamId = note.get('teamId')
    #     else:
    ...
    #         teamId = ''
    #     playerId_li = self.col_read_player.find({'teamId':teamId,'nameZHAbbr': playerName},{'playerId':1,'nameEN':1})
    #     if  len(list(playerId_li)) > 1 :
    #         for i in playerId_li:
    #             if i.get('nameEN') == playerName_en:
    #                 playerId = i.get('playerId')
    #                 return playerId
    #     elif len(list(playerId_li)) == 1:
    #         playerId = playerId_li[0].get('playerId')
    #     return playerId
    ...
    def get_player_id(self, playerName_1,playerName_en,teamId):  # resp是一个元组
        note = self.col_read_player.find_one({'nameZH': {'$regex': playerName_1}, 'teamId':teamId},
                                             {'playerId': 1, 'teamBelongs': 1})
        if not note:
            if teamId:
                note = self.col_read_player.find_one({'nameEN': playerName_en,'teamId':teamId}, {'playerId': 1})
            else:
                note = self.col_read_player.find_one({'nameEN': playerName_en}, {'playerId': 1})
        if note:
    ...
    def parse_transfer(self,playerid,playerId,playerName,arg): #球员转会 transferMarket
        ...
        for i in range(1,len(tr)+1):
            season = self.is_empty(self.my_xpath(resp,'//div[@class="row"]/div[1]/table/tbody/tr[{}]/td[1]/text()'.format(i)))
            date = self.is_empty(self.my_xpath(resp,'//div[@class="row"]/div[1]/table/tbody/tr[{}]/td[2]/text()'.format(i)))
            leftTeamName = self.is_empty(self.my_xpath(resp,'string(//div[@class="row"]/div[1]/table/tbody/tr[{}]/td[3])'.format(i))).replace('\r','').replace('\n','').replace('\t','')
            leftTeamId = self.get_team_id(leftTeamName)
            joinedTeamName = self.is_empty(self.my_xpath(resp,'string(//div[@class="row"]/div[1]/table/tbody/tr[{}]/td[4])'.format(i))).replace('\r','').replace('\n','').replace('\t','')
            joinedTeamId = self.get_team_id(joinedTeamName)
            marketValue = self.is_empty(self.my_xpath(resp,'//div[@class="row"]/div[1]/table/tbody/tr[{}]/td[5]/text()'.format(i)))
            transferFee = self.is_empty(self.my_xpath(resp,'//div[@class="row"]/div[1]/table/tbody/tr[{}]/td[7]/text()'.format(i)))
            item = {
                    'playerId':playerId,
                    'playerName':playerName,
                    'playerName_en':playerName_en,
                    'teamId':teamId,
                    'teamName':teamName,
                    'countryZh':countryZh,
    ...
    def playerMarketValue(self,data,playerId,playerName,arg): #球员身价浮动
        ...
        playerMarketValue = {
            ...
            'highestWorth':highestWorth,
            'currentDate':currentDate,
            'currentWorth':currentWorth,
            'marketValueDetail':marketValueDetail,
        }
    ...
    def player_detail(self,playerid,playerId,season,playerName,arg): #球员详细数据
        ...
        item = {'trend trend-hugerise':2,'trend trend-rise':1,'trend trend-flat':0,'trend trend-fall':-1,'trend trend-hugefall':-2,'':''}
        player_detail_item = {
                                'playerId':playerId,
                                'season':season,
                                'playerName':playerName,
            ...
            for r in range(1,len(row_statPre)+1):
                if r == 1 or r == 2: #前俩栏数据
                    ...
                    for d in range(1,len(text_div)+1):
                        table_tr = self.my_xpath(resp, '//div[@class="smallTab"]/div[{}]/div[{}]/div[{}]/table/tbody/tr'.format(i, r, d))
                        for x in range(1,len(table_tr)+1):
                            ...
                            if r == 1 and d == 1 and x == 1: #红黄牌
                                name = self.my_xpath(resp, '//div[@class="smallTab"]/div[{}]/div[{}]/div[{}]/table/tbody/tr[{}]/td[1]/text()'.format(i, r, d, x))[0]
                                ability = self.my_xpath(resp, '//div[@class="smallTab"]/div[{}]/div[{}]/div[{}]/table/tbody/tr[{}]/td[2]/span/text()'.format(i, r, d, x))
                                teamRank = self.my_xpath(resp, '//div[@class="smallTab"]/div[{}]/div[{}]/div[{}]/table/tbody/tr[{}]/td[3]/span/text()'.format(i, r, d, x))
                                teamRank = ''.join(teamRank).replace('\r', '').replace('\n', '').replace('\t', '').replace(' ', '')
                                positionRank = self.my_xpath(resp, '//div[@class="smallTab"]/div[{}]/div[{}]/div[{}]/table/tbody/tr[{}]/td[4]/span/text()'.format(i, r, d, x))
                                ...
                                positionRank = self.my_xpath(resp,'//div[@class="smallTab"]/div[{}]/div[{}]/div[{}]/table/tbody/tr[{}]/td[4]/span/text()'.format(i, r, d, x))
                                positionRank = ''.join(positionRank).replace('\r', '').replace('\n', '').replace('\t','').replace(' ', '')
                                li = [self.get_interger(ability_1),self.get_interger(ability_2),self.get_interger(teamRank),self.get_interger(positionRank)]
                                data_item[itemName.get(name,name)] = li

                            else: #其他
                                name = self.my_xpath(resp, '//div[@class="smallTab"]/div[{}]/div[{}]/div[{}]/table/tbody/tr[{}]/td[1]/text()'.format(i, r, d, x))[0]
                                ability = self.my_xpath(resp,'//div[@class="smallTab"]/div[{}]/div[{}]/div[{}]/table/tbody/tr[{}]/td[2]/span[1]/text()'.format(i, r, d, x))
                                ability = ''.join(ability).replace('\r', '').replace('\n', '').replace('\t','').replace(' ','')
                                trend  = self.is_empty(self.my_xpath(resp,'//div[@class="smallTab"]/div[{}]/div[{}]/div[{}]/table/tbody/tr[{}]/td[2]/span[2]/@class'.format(i, r, d, x)))
                                teamRank = self.my_xpath(resp,'//div[@class="smallTab"]/div[{}]/div[{}]/div[{}]/table/tbody/tr[{}]/td[3]/span/text()'.format(i, r, d, x))
                                teamRank = ''.join(teamRank).replace('\r', '').replace('\n', '').replace('\t','').replace(' ', '')
                                positionRank = self.my_xpath(resp,'//div[@class="smallTab"]/div[{}]/div[{}]/div[{}]/table/tbody/tr[{}]/td[4]/span/text()'.format(i, r, d, x))
                                positionRank = ''.join(positionRank).replace('\r', '').replace('\n', '').replace('\t','').replace(' ', '')
                                li = [self.get_interger(ability), item.get(trend), self.get_interger(teamRank),self.get_interger(positionRank)]
                                data_item[itemName.get(name,name)] = li
                else: #后三栏的数据
                    text_tr = self.my_xpath(resp, '//div[@class="smallTab"]/div[{}]/div[{}]/div[1]/table/tbody/tr'.format(i, r))
                    for tr in range(1,len(text_tr)+1):
                        name = self.my_xpath(resp, '//div[@class="smallTab"]/div[{}]/div[{}]/div[1]/table/tbody/tr[{}]/td[1]/text()'.format(i, r, tr))[0]
                        if name == '把握机会能力':
                            name = '把握机会能力1'
                        elif name == '传球成功率':
                            name = '传球成功率1'
                            ...
                            name = '前场传球成功率1'
                        value_1 = self.my_xpath(resp, '//div[@class="smallTab"]/div[{}]/div[{}]/div[1]/table/tbody/tr[{}]/td[2]/span/span[1]/span/text()'.format(i, r, tr))
                        value_1 = ''.join(value_1).replace('\r', '').replace('\n', '').replace('\t','').replace(' ', '')
                        value_2 = self.my_xpath(resp, '//div[@class="smallTab"]/div[{}]/div[{}]/div[1]/table/tbody/tr[{}]/td[2]/span/span[2]/text()'.format(i, r, tr))
                        value_2 = ''.join(value_2).replace('\r', '').replace('\n', '').replace('\t', '').replace(' ', '')
                        trend = self.is_empty(self.my_xpath(resp, '//div[@class="smallTab"]/div[{}]/div[{}]/div[1]/table/tbody/tr[{}]/td[4]/span/@class'.format(i, r, tr)))
                        teamRank = self.my_xpath(resp, '//div[@class="smallTab"]/div[{}]/div[{}]/div[1]/table/tbody/tr[{}]/td[5]/span/text()'.format(i, r, tr))
                        teamRank = ''.join(teamRank).replace('\r', '').replace('\n', '').replace('\t', '').replace(' ', '')
                        positionRank = self.my_xpath(resp,'//div[@class="smallTab"]/div[{}]/div[{}]/div[1]/table/tbody/tr[{}]/td[6]/span/text()'.format(i, r, tr))
                        positionRank = ''.join(positionRank).replace('\r', '').replace('\n', '').replace('\t', '').replace(' ', '')
                        li = [self.get_interger(value_1),self.get_interger(value_2),item.get(trend),self.get_interger(teamRank),self.get_interger(positionRank)]
                        data_item[itemName.get(name,name)] = li
        ...
        if playerId:
            ...
            if not season:
                ...
                if not note:
                    ...
                    if playerid:  # and not teamData['teamId']
                        playerData['playerId'] = playerid
                        collect.update_one({'playerName': playerData.get('playerName'), 'season': season}, {'$set': playerData})
    ...
    def spider_player(self,name,id,teamName,countryZh,playerMP):
        logger.info('{}：数据开始下载'.format(id))
        try:
            playerid = int(id)
            ...
            playerName_en = self.is_empty(self.my_xpath(resp, '//div[@class="player-info"]/div/div[1]/table//tr[1]/td/text()'))
            playerId = self.get_player_id(name, playerName_en, teamId)
            
            args = (playerName_en,teamId,teamName,countryZh,playerMP)
            ...
            logger.info('playerid:{},数据入库失败-->{}'.format(id,e))


    def run(self):
        for name,id,teamName,countryZh,playerMP in  self.get_playerUrl():
            self.pool.apply_async(self.spider_player,args=(name,id,teamName,countryZh,playerMP))
            # self.spider_player(name,id,teamName)
        self.pool.close()
        self.pool.join()
        
    def take_player_info(self):
        for i in range(1,14):
    ...
    def get_playerUrl(self):
        ...
        url_con = 'http://www.tzuqiu.cc/playerStatistics/querys.json?start=0&length={}&extra_param%5BisCurrentSeason%5D=true&extra_param%5BcompetitionRange%5D=all&extra_param%5BorderCdnReq%5D=true&extra_param%5BsatisfyAttendance%5D=true'.format(count)
