...
class Team:
    ...
    def save_database(self, team_id, data):
        ...
        for i in data['honor']:
            ...
            img_src = etree.HTML(resp).xpath('/html/body/img/@src')
            if img_src:
                i['logo'] = logo_url
            honor_list.append(i)   
    ...
    def crm(self,teamid):
        ...
        self.save_database(teamid,detail_page_info)
        if detail_page_info.get('_id','f') != 'f':
            detail_page_info.pop('_id')
        # except Exception as e:
            # detail_page_info = {'error':'数据获取失败，请再次访问！'}
        print(detail_page_info)
