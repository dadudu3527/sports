...
fh = logging.FileHandler('C:\\sports\\football\\data\\renewLeague.log')
...
class Team:
    def __init__(self):
        ...
        self.collection_p = dbname_l['player']
    ...
    def renew_league(self,team_id): #更新球队所属联赛
        try:
            ...
            response = etree.HTML(resp) 
            self.renew_league2(team_id,response) #球员添加国家队
            team_info = self.is_empty(response.xpath('//div[@class="clearfix-row bd-box info"]/ul[@class="info-data"]/li[1]/span[2]/text()'))
            if team_info:
                leagueId_note = self.collection_l.find_one({'short_name_zh':team_info},{'leagueId':1})
                if leagueId_note:
                    leagueId = leagueId_note.get('leagueId')
                    team_note = self.collection_league.find_one({'teamId':team_id})
                    if team_note:
                        self.collection_league.update_one({'teamId':team_id},{'$set':{'leagueId':leagueId}})
                        logger.info('{}:球队所属赛季更新成功！{}'.format(team_id,leagueId))
        except Exception as e:
            self.mdb.sadd('teamLeague',team_id)
    ...
    def player_ur(self, response):  # 球员url
        # response = etree.HTML(data)
    ...
    def renew_league2(self,team_id,response): #球员添加国家队
        # url = 'https://data.leisu.com/team-{}'.format(team_id)
        # resp = self.get_data(url)
        # response = etree.HTML(resp) 
        team_info = self.is_empty(response.xpath('//div[@class="clearfix-row bd-box info"]/ul[@class="info-data"]/li[1]/span[1]/text()'))
        team_name = ''.join(response.xpath('/html/body/div[1]/div[2]/div[2]/div[1]/div[1]/div[2]/p[1]/span/text()'))
        if team_info != '所属联赛:':
            players = self.player_ur(response)
            for i in players:
                n_player = self.collection_p.find_one({'playerId':i})
                if n_player:
                    self.collection_p.update_one({'playerId':i},{'$set':{'countryTeamId':team_id,'countryTeamName':team_name}})
    ...
    def logo_url(self, url, teamid): #更换球队logo，需要先删除数据
    ...
    def crm(self,teamid):
        try:
            ...
            item = {'error':'数据获取失败，请再次访问！'}
        return item

    def swap_league(self):  # 52054
        for i in range(10000,52054):
            self.pool.apply_async(self.renew_league, args=(i,))
        self.pool.close()
        self.pool.join()
    ...
    def read_redis(self):
        print('开始redis读取。。。')
        while True:
            teamId = self.mdb.spop('teamLeague')
            if teamId:
                self.renew_league(int(teamId))
            else:
                break
