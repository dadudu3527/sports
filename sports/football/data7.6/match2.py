...
class Match:
    ...
    def getProxy(self):
        url = 'http://mapeng1114.v4.dailiyun.com/query.txt?key=NP95DAADAE&word=&count=100&rand=true&detail=false'
        headers = {
    ...
    def get_data(self,url):
        ...
        resp = requests.get(url=url,headers=self.headers,proxies=proxies).text
        return resp
    ...
    def parse_data_saiguo(self,seasonId): #赛程赛果
        try:
            ...
            if leagueId != '':
                ...
                seasonid = self.errorCol.find_one({'seasonId': seasonId}) #代表数据库没有leagueId
                if not seasonid:
                    self.errorCol.insert_one({'seasonId': seasonId,'type':1})

            logger.info('{}数据下载结束'.format(seasonId))

        except Exception as e:
            seasonid = self.errorCol.find_one({'seasonId': seasonId})
            if not seasonid:
                self.errorCol.insert_one({'seasonId':seasonId,'type':2}) #type=2代表数据解析异常
