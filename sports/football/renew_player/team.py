...
fh = logging.FileHandler('C:\\sports\\football\\data\\team.log')
...
class Team:
    def __init__(self):
        ...
        self.teamId_list = []
        note = self.collection_league.find({'leagueId':2773},{'teamId':1})
        for i in note:
            teamid = i.get('teamId')
            self.teamId_list.append(teamid)    
    ...
    def run_team2(self,url,team_id):
        try:
            ...
            #detail_page_info = self.detail_page(response, team_id)
            if team_id not in self.teamId_list:
                Player().thread_pool(player_url,team_id)
