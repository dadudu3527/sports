...
class LeisuSpiderSpider(scrapy.Spider):
    ...
    def detail_parse(self,response):
        detail_name = response.xpath('//div[@class="clearfix-row f-s-24"]/text()').extract_first()
        ...
        for time_zone,time_url in zip(time_zone_list,time_url_list):
            seasonId = int(time_url.split('-')[-1])
            # li[seasonId] = {'seasonId':seasonId,'leagueName':leagueName,'seasonYear':time_zone}
            ...
            detail_item['seasonId'] = seasonId
            detail_item['area'] = area
            detail_item['country'] = country
            detail_item['leagueName'] = leagueName
            detail_item['leagueFullName'] = detail_name
            detail_item['seasonYear'] = time_zone
            yield detail_item

        # matchRule = response.xpath('//div[@class="clearfix-row f-s-12 p-t-15 line-h-22 color-666"]/text()').extract()
        # item = RuleSpiderItem()
        # item['leagueId'] = int(detail_item_seasonid)
        # item['season'] = li
        # item['matchRule'] = matchRule
        #
        # yield item
