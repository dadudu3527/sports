...
class TiyuData:
        ...
        if not div_list:
            ...
            for y, lun in enumerate(all_score):  # y索引all_score列表
                ...
                for div in range(1, len(div_list) + 1):
                    ...
                    data = self.collection_jf.find_one({'seasonId': seasonId,'round':items.get('round'),'type':div})
                    if not data:
                        self.collection_jf.insert_one(items)
                    else:
                        self.collection_jf.update_one({'seasonId': seasonId,'round':items.get('round'),'type':div}, {'$set': items})
                    # print(items)
        ...
        if not div_list:
            ...
            for y in range(2):  # 总轮次和当前最后一轮
                ...
                try:
                    ...
                    res.click()
                try:
                    lunci = wait.until(EC.element_to_be_clickable((By.XPATH,'//div[@class="table-list"]/div[1]/div/div/div[1]/ul/li[{}]/a'.format(dic.get(y)))))  # 实现轮次的点击
                    lunci.click()
                except:
                    lunci = wait.until(EC.element_to_be_clickable((By.XPATH,'//div[@class="table-list"]/div[1]/div/div/div[1]/ul/li[{}]'.format(dic.get(y)))))  # 实现轮次的点击
                    lunci.click()
    ...
    def parse_data(self, note):
        ...
        self.get_data(url)
        leagueId = self.getLeagueId(seasonId) #取消注释
        # leagueId = '' #去掉
        try:
...
#     def nq(*args):
#         time1 = datetime.now()
#         times1 = (time1.hour,time1.minute,time1.second)
#         a(*args)
#         time2 = datetime.now()
#         times2 = (time2.hour,time2.minute,time2.second)
#         time_h,time_m,time_s = (times2[0]-times1[0],times2[1]-times1[1],times2[2]-times1[2])
#         if time_h == 0:
#             print('下载时间为：{}分{}秒'.format(time_m,time_s))
#         else:
#             print('下载时间为：{}时{}分{}秒'.format(time_h,time_m, time_s))
#     return nq
...
if __name__ == "__main__":
    ...
    def func(seasonId):
        a = TiyuData()
        # item = a.requests_jifen(seasonId,'')
        # print(item)
        a.parse_data(seasonId)
        a.exit_chrome()  # a.thread_pool2()
    func(8828)